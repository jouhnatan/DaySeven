...
class FakeAssetRepository extends AssetRepository {
  @override
  Future<void> uploadReferenced({required KnowledgeBase kb, required BlockDocument document}) async {}
  @override
  Future<void> downloadMissing({required KnowledgeBase kb, required BlockDocument document}) async {}
}
...
class FakeDocuments extends DocumentRepository {
  ...
  Future<RemoteDocumentSnapshot?> snapshotForDocument(String documentId) async =>
      byId[documentId] ?? snapshots.where((s) => s.document.id == documentId).firstOrNull;
  ...
  @override
  Future<String?> currentRevisionId(String documentId) async => byId[documentId]?.revisionId;
...
void main() {
  ...
  group('KbHierarchyReplicator', () {
    ...
    setUp(() async {
      ...
      kbA = await KnowledgeBase.create(folder: tempA.path, name: 'Awayside', kbId: kbId);
      kbB = await KnowledgeBase.create(folder: tempB.path, name: 'Awayside', kbId: kbId);
    ...
    Future<ProviderContainer> makeContainer(KnowledgeBase kb, SearchIndex index, FakeDocuments docs) async {
      ...
      return ProviderContainer(overrides: [
        ...
        appStoreProvider.overrideWith((ref) async => AppStore(File('${kb.rootPath}.app-store.json'))),
      ]);
    }
    ...
    test('publishing Untitled in Awayside replicates hierarchy and data on peer', () async {
      ...
      final pathA = await kbA.createDocument(title: 'Untitled', folderRelativePath: 'Awayside');
      ...
      final fakeDocs = FakeDocuments(snapshots: [
        RemoteDocumentSnapshot(path: pathA, revisionId: 'rev-1', document: docA),
      ]);

      // Device B: empty, no Awayside folder
      expect(await Directory('${tempB.path}/Awayside').exists(), isFalse);
      expect(await File('${tempB.path}/Awayside/Untitled.md').exists(), isFalse);
    ...
    test('nested hierarchy Awayside/Lore/Untitled replicates parents recursively', () async {
      await kbA.createFolder('Awayside/Lore');
      final pathA = await kbA.createDocument(title: 'Untitled', folderRelativePath: 'Awayside/Lore');
      expect(pathA, 'Awayside/Lore/Untitled.md');
      final docA = await kbA.readDocument(pathA);
      final fakeDocs = FakeDocuments(snapshots: [
        RemoteDocumentSnapshot(path: pathA, revisionId: 'rev-nested', document: docA),
      ]);
      final containerB = await makeContainer(kbB, indexB, fakeDocs);
      addTearDown(containerB.dispose);
      final replicator = containerB.read(kbHierarchyReplicatorProvider);
      ...
      expect(await File('${tempB.path}/Awayside/Lore/Untitled.md').exists(), isTrue);
    ...
    test('does not overwrite divergent local file – counts as conflict', () async {
      ...
      final canonicalDoc = (await kbA.readDocument(pathA)).copyWith(
        blocks: [ParagraphBlock(id: 'p1', spans: [TextSpanNode(text: 'Canonical')])],
      );
      await kbA.writeDocument(pathA, canonicalDoc);
      final fakeDocs = FakeDocuments(snapshots: [
        RemoteDocumentSnapshot(path: pathA, revisionId: 'rev-2', document: canonicalDoc),
      ]);
      // Device B has same path but divergent local edit, with ledger at rev-1
      await kbB.createFolder('Awayside');
      final pathB = await kbB.createDocument(title: 'Untitled', folderRelativePath: 'Awayside');
      ...
      final divergent = BlockDocument(
        id: canonicalDoc.id,
        title: 'Untitled',
        blocks: [ParagraphBlock(id: 'p1', spans: [TextSpanNode(text: 'Divergent local edits')])],
      );
      await kbB.writeDocument(pathB, divergent);
      final ledgerB = await SyncLedger.open(kbB);
      await ledgerB.record(document: divergent, revisionId: 'rev-1', path: pathB);
      ...
      final oldBase = BlockDocument(id: canonicalDoc.id, title: 'Untitled', blocks: [ParagraphBlock(id: 'p1', spans: [TextSpanNode(text: 'Base')])]);
    ...
    test('ensureRemoteMatchesLocal publishes missing local hierarchy to canonical', () async {
      // Device A has local hierarchy Awayside/Untitled.md not yet in snapshot
      await kbA.createFolder('Awayside');
      final pathA = await kbA.createDocument(title: 'Untitled', folderRelativePath: 'Awayside');
      final docA = await kbA.readDocument(pathA);
      final docs = FakeDocuments(snapshots: []);
      final sessionA = KbSession(kb: kbA, index: indexA, tree: await kbA.readTree());
      final containerA = ProviderContainer(overrides: [
        ...
        differencesNetworkEnabledProvider.overrideWithValue(true),
        differencesRealtimeEnabledProvider.overrideWithValue(false),
        appStoreProvider.overrideWith((ref) async => AppStore(File('${tempA.path}.app-store.json'))),
      ]);
    ...
    test('reconcile pulls then pushes', () async {
      ...
      final loreDoc = BlockDocument(id: 'lore-id', title: 'Lore', blocks: [ParagraphBlock(id: 'p2', spans: [TextSpanNode(text: 'Lore text')])]);
      final fakeDocs = FakeDocuments(snapshots: [
        RemoteDocumentSnapshot(path: 'Lore.md', revisionId: 'rev-lore', document: loreDoc),
      ]);
      ...
      expect(fakeDocs.published.any((s) => s.path == 'Awayside/Untitled.md'), isTrue);
    });
  });
}
